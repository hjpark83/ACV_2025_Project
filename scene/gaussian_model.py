...
class GaussianModel:
    ...
    def initialize_dino_features(self, dino_features, feature_dim=384, requires_grad=True):
        ...
        if dino_features is not None:
            ...
            print(f"✓ Initialized DINOv2 features: {self._dino_features.shape}")
            ...
            print(f"⚠ Randomly initialized DINOv2 features: {self._dino_features.shape}")
            self.dino_reliability_weights = torch.ones(N, device="cuda")
    ...
    def training_setup(self, training_args):
        ...
        if hasattr(self, '_dino_features') and self._dino_features.numel() > 0:
            ...
            print(f"✓ Added DINO features to optimizer (lr={training_args.feature_lr / 2.0:.6f})")
    ...
    def load_ply(self, path):
        ...
        if len(dino_names) > 0:
            ...
            print(f"✓ Loaded DINO features: {self._dino_features.shape}")
        else:
            self._dino_features = torch.empty(0)
            self.dino_feature_dim = 0
            print("⚠ No DINO features found in checkpoint")
        ...
        if 'mask_id' in [p.name for p in plydata.elements[0].properties]:
            ...
            print(f"✓ Loaded mask_ids: {self.mask_ids.shape}, unique IDs: {len(torch.unique(self.mask_ids[self.mask_ids >= 0]))}")
        else:
            print("⚠ No mask_ids found in checkpoint")
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_objects_dc, new_dino_features=None, new_dino_weights=None, new_mask_ids=None):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_objects_dc, new_dino_features, new_dino_weights, new_mask_ids)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_objects_dc, new_dino_features, new_dino_weights, new_mask_ids)
